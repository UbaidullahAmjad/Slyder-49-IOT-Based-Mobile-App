...
import {
  ...
  SAILING_CENTER_T,
  SAILING_DTW,
  CHANGE_SAILING_RIGHT_SOG,
  CHANGE_SAILING_HEADING,
  CHANGE_RUDDER_POSITION,
  SAILING_CENTER_WATER_TEMP,
  SAILING_CENTER_DEPT,
  CHANGE_SAILING_LEFT_SPEED,
  CHANGE_SAILING_ETA,
  NORTH_DIRECTION,
  WEST_DIRECTION,
} from '../../../type';

export const N2KSailing = (split, dispatch) => {
  if (split[6]) {
    var angle = split[6];
  } else {
    var angle = 0;
  ...
  if (split[0] == 'n2k' && split[1] == 'wind_data') {
    ...
    if (store.getState().SailingCenter.waterTemp != split[5]) {
      dispatch({
        type: SAILING_CENTER_WATER_TEMP,
        payload: split[5],
      });
    }
    ...
    if (store.getState().SailingLeft.SailingSpeed != split[5])
      dispatch({
        type: CHANGE_SAILING_LEFT_SPEED,
        payload: split[5],
      });
  }
};
